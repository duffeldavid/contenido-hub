...
- **Mercadeo GM → David**: aprobaciones y comentarios (`tipo: "aprob"`), con push al celular por el canal de notificaciones. En el link del cliente cada pieza tiene una sola caja **Comentario para David**: cuadro de texto y botón **Guardar comentario** siempre visible (apagado hasta que hay texto). Lo que va escribiendo queda como borrador en el navegador y sobrevive a cualquier cambio de estado, repintado o recarga. Si marca Aprobado / Ajustar / Pendiente con un comentario nuevo escrito, el comentario se guarda y viaja junto con el estado (una sola acción); si está cambiando un comentario ya guardado, el cambio queda como borrador hasta que toque **Guardar cambios**. Un comentario guardado se ve como *Tu comentario · Guardado · David ya lo ve* y solo cambia con **Editar** o se quita con **Borrar** (con confirmación; viaja como `borrarC: true`). Dentro de las pestañas Aprobadas / Con ajustes / Pendientes las piezas no desaparecen al cambiarles el estado: se quedan con la etiqueta *Ahora está en …* hasta que se vuelve a tocar una pestaña o se cambia de marca. En la plataforma de David un evento `aprob` con comentario vacío conserva el comentario que ya había: el comentario de mercadeo solo cambia cuando llega uno nuevo o cuando mercadeo lo borra a propósito.
