...
videoPicker.onchange = () => {
  const f = videoPicker.files[0];
  if (!f || !videoPickTarget) return;
  const pr = progDe(videoPickTarget);
  pr.video = f.name;
  pr.img = null;
  marcarPendiente(); save(); renderHistorias();
  abrirHistoriaClave(videoPickTarget, "video");
};
function openHistoria(iso, mk, txt, medioVista) {
  ...
  drawer.innerHTML = `
    ...
    <section class="pub-paso pub-auto" style="margin-top:18px">
      ...
      : `
      <p class="pub-nota">El video final (<b>MP4 o MOV vertical, máx. 60 s</b>) va en la carpeta <b>Videos Contenido Hub</b> del escritorio de tu Mac. Elígelo aquí — solo se toma el nombre; el Mac lo sube y lo publica <b>tal cual, sin nada encima</b>.</p>
      <div class="hist-prog-grid">
        <div class="hist-img-prev vacia hist-video-prev">
          ${pr && pr.video ? `<span class="hist-play">${iconoHist('<path d="M8.5 5.5v13l10-6.5-10-6.5z"/>', "#fff", "hist-ic hist-ic-play")}</span><span class="hist-video-nombre">${esc(pr.video)}</span>` : `<span>MP4<br>9:16</span>`}
        </div>
        <div class="hist-prog-campos">
          <button class="btn-primary" id="histElegirVideo">${pr && pr.video ? "Cambiar video" : "Elegir el video…"}</button>
          <label class="hist-lbl">O escribe el nombre del archivo</label>
          <input id="histVideoNombre" class="edit-input" placeholder="mi-historia.mp4" value="${esc((pr && pr.video) || "")}" autocomplete="off" spellcheck="false">
          ${campoHoraRedes(pr)}
        </div>
      </div>`}
      ...
      <p class="${lista ? "hist-estado-ok" : "pub-aviso"}" style="margin-top:12px">
        ${lista
          ...
          : `Sube la imagen o elige el video y quedará programada — sin eso no se publica.`}
      </p>
      ${pr ? `<button class="link-btn" id="histQuitarProg" style="color:#B23A2E;margin-top:6px">Quitar programación</button>` : ""}
    </section>
  ...
  if (inVid) inVid.onchange = () => {
    const pr2 = progDe(k);
    pr2.video = inVid.value.trim() || null;
