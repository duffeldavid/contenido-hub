...
// La cola ligera para el Mac: lo que el trabajador necesita para publicar
// (sin el peso de todas las portadas — solo las de piezas encoladas).
function colaPublicable(ts) {
  const portadas = {};
  for (const id of Object.keys(store.meta)) {
    if (store.portadas[id]) portadas[id] = store.portadas[id];
  }
  return {
    ts: ts || Date.now(),
    meta: store.meta, historias: store.historias,
    fechas: store.fechas, horas: store.horas, ediciones: store.ediciones,
    estados: store.estados, ocultas: store.ocultas, nuevas: store.nuevas,
    portadas,
  };
}
async function publicarCambios() {
  if (EN_ARTIFACT) {
    // El visor de claude.ai bloquea las conexiones externas, pero el canal
    // interno del artifact (db) sí funciona: la cola viaja por ahí y el
    // puente la reenvía al Mac en unos minutos.
    const b = document.getElementById("btnGuardarCambios");
    if (b) { b.disabled = true; b.querySelector(".guardar-txt").textContent = "Guardando…"; }
    try {
      const db = await window.claude.use("db");
      if (!db) throw new Error("sin canal");
      let cola = colaPublicable();
      let cuerpo = JSON.stringify(cola);
      if (cuerpo.length > 800000) {
        await reducirPortadas();
        cola = colaPublicable(cola.ts);
      }
      await db.doc("cola/actual").set(cola);
      store.pendientePub = false;
      store.pubTs = cola.ts;
      save();
      toastVivo("✓ Guardado desde claude.ai — el Mac recibe la cola en unos minutos");
    } catch (e) {
      toastAccion("No se pudo guardar aquí — usa el enlace público", "Abrir", () => {
...
function aplicarEstado(est, avisar) {
  ...
  // La aprobación de mercadeo entra SIEMPRE — aun con cambios locales sin
  // guardar o un estado viejo. Solo rellena lo que falte (lo local gana):
  // así los veredictos jamás quedan atrapados detrás de los guardias.
  let aprobRecuperada = false;
  for (const grupo of ["aprob", "checks"]) {
    const inc = est[grupo];
    if (!inc) continue;
    for (const id of Object.keys(inc)) {
      if (!(id in store[grupo])) { store[grupo][id] = inc[id]; aprobRecuperada = true; }
    }
  }
  const bloqueado = est.ts <= (store.pubTs || 0) ||      // ya tenemos esta versión
    (!MODO_CLIENTE && store.pendientePub);               // no pisar cambios locales sin guardar
  if (bloqueado) {
    if (aprobRecuperada) { save(); renderAll({ keep: true }); }
    return false;
  }
...
// Íconos de línea minimalistas (estilo app moderna) — en TODA la plataforma:
// cero emojis genéricos (decisión de diseño de David).
const ICOL = {
  ...
  foto: '<svg class="icl" viewBox="0 0 24 24"><rect x="3" y="6" width="18" height="14" rx="3"/><circle cx="12" cy="13" r="4"/><path d="M9 6l1.5-2h3L15 6"/></svg>',
  capas: '<svg class="icl" viewBox="0 0 24 24"><rect x="7" y="7" width="14" height="14" rx="3"/><path d="M3 15V6a3 3 0 0 1 3-3h9"/></svg>',
  rayo: '<svg class="icl" viewBox="0 0 24 24"><path d="M13 2 5 13h6l-1 9 8-11h-6l1-9z"/></svg>',
  caja: '<svg class="icl" viewBox="0 0 24 24"><path d="M21 8 12 3 3 8v8l9 5 9-5V8z"/><path d="m3 8 9 5 9-5M12 13v8"/></svg>',
  nube: '<svg class="icl" viewBox="0 0 24 24"><path d="M7 18a4.5 4.5 0 0 1-.7-8.95A6 6 0 0 1 17.8 10.6 3.8 3.8 0 0 1 17 18H7z"/></svg>',
  descarga: '<svg class="icl" viewBox="0 0 24 24"><path d="M12 3v11M7 10l5 5 5-5M4 20h16"/></svg>',
  copiar: '<svg class="icl" viewBox="0 0 24 24"><rect x="9" y="9" width="12" height="12" rx="2.5"/><path d="M5 15H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h9a2 2 0 0 1 2 2v1"/></svg>',
  ...
  paleta: '<svg class="icl" viewBox="0 0 24 24"><path d="M12 21a9 9 0 1 1 9-9c0 2.2-1.5 3.4-3 3.4h-2.2a2 2 0 0 0-1.4 3.4c.6.6.2 2.2-2.4 2.2z"/><circle cx="7.6" cy="11" r="1"/><circle cx="11" cy="7.6" r="1"/><circle cx="15.6" cy="9.4" r="1"/></svg>',
  hoja: '<svg class="icl" viewBox="0 0 24 24"><path d="M4 20C6 8 12 4 20 4c0 10-6 15-14 15"/><path d="M4 20c2-5 5-8 10-10"/></svg>',
  camara: '<svg class="icl" viewBox="0 0 24 24"><rect x="3" y="10" width="18" height="10" rx="2.5"/><path d="M3.6 6.5 20 4l.5 3.5L4.1 10z"/></svg>',
};
const icl = n => ICOL[n] || "";
// Ícono por formato de pieza (miniaturas del planificador)
const FORMATO_IC = { "Reel": "video", "Foto": "foto", "Carrusel": "capas", "Pieza gráfica": "ajuste", "Historia": "rayo" };
    ...
    html += `
      <div class="flujo-col" data-destino="${col.destino}">
        <div class="flujo-col-head">
          <span class="flujo-ico" style="color:${col.color}">${icl(col.ic)}</span>
...
function renderAprobacion() {
  ...
  el.querySelectorAll(".aprob-row").forEach(row => {
    ...
    if (pdfCb) pdfCb.onchange = () => {
      ...
      if (btn) btn.innerHTML = `${icl("descarga")} PDF cliente (${todas.filter(x => store.pdf[x.id] !== false).length})`;
    };
  });
  ...
  if (btnLink) btnLink.onclick = async () => {
    ...
    try {
      ...
      btnLink.innerHTML = `${icl("ok")} Link copiado`;
      setTimeout(() => { btnLink.innerHTML = `${icl("copiar")} Link cliente`; }, 2500);
    } catch {
...
fileInput.onchange = async () => {
  ...
  try {
    if (histImgTarget) {
      ...
      save();
      renderHistorias();
      abrirHistoriaClave(histImgTarget, "imagen");
      return;
    }
...
function openDrawer(id) {
  ...
  drawer.innerHTML = `
    ...
    <section>
      ...
      <button class="btn-programar" id="btnIrPublicar" style="margin-top:10px">${icl("enviar")} Programar en Meta</button>
    </section>
...
function openPublicar(id) {
  ...
  drawer.innerHTML = `
    ...
    <span class="chip brand" style="--brand-color:${m.color};--brand-tint:${brandTint(p)}">${m.nombre} · ${m.handle}</span>
    <h2>${esc(tituloDe(p))}</h2>
    <div class="sub">${p.formato} · ${dia.toLowerCase()} ${num}</div>
    ...
    <section class="pub-paso">
      ...
      <div class="portada-box">
        ...
        <div class="portada-acts">
          ...
          ${img ? `<button class="btn-primary" id="pubDescargar">${icl("descarga")} Descargar</button>` : ""}
        </div>
      </div>
    </section>

    <section class="pub-paso">
      <h4><span class="paso-num">2</span> Copy</h4>
      <textarea id="pubCopy" class="aprob-comment" style="min-height:110px">${esc(copyDe(p))}</textarea>
      <button class="btn-primary" id="pubCopiar" style="margin-top:8px">${icl("copiar")} Copiar copy</button>
    </section>
    ...
    <section class="pub-paso pub-auto">
      <h4><span class="paso-num auto">A</span> Automática</h4>
      <label class="auto-check">
        <input type="checkbox" id="pubAuto" ${metaDe(p) && metaDe(p).auto ? "checked" : ""}>
        <span>Publicar sola desde el Mac</span>
      </label>
      ...
      <p class="pub-nota" style="margin:10px 0 0">Facebook queda agendado en Meta; Instagram sale a la hora exacta. Guarda cambios.</p>
      ${!img ? `<p class="pub-aviso" ${metaDe(p) && metaDe(p).auto ? "" : "hidden"} id="pubAvisoImg">Instagram necesita la portada — sin ella solo sale en Facebook.</p>` : ""}
    </section>

    <section class="pub-paso">
      <h4><span class="paso-num">4</span> Manual en Meta</h4>
      <div class="pub-meta-btns">
        <a class="btn-meta" id="pubMeta" href="${META_COMPOSER}" target="_blank" rel="noopener">${icl("enviar")} Abrir Meta</a>
        <a class="btn-ghost" href="${META_PLANNER}" target="_blank" rel="noopener">Calendario de Meta</a>
      </div>
    </section>

    <section class="pub-paso">
      <h4><span class="paso-num">5</span> Confirmar</h4>
      <div class="aprob-pills">
        <button id="pubMarcarProg" class="${est === "Programado" ? "sel" : ""}">${icl("reloj")} Programada</button>
        <button id="pubMarcarPub" class="${est === "Publicado" ? "sel" : ""}">${icl("ok")} Publicada</button>
      </div>
    </section>`;
  ...
  if (bDesc) bDesc.onclick = () => { descargarPortada(p); bDesc.innerHTML = `${icl("ok")} Descargada`; };
  ...
  drawer.querySelector("#pubCopiar").onclick = async function () {
    try {
      ...
      this.innerHTML = `${icl("ok")} Copiado — pégalo en Meta`;
    } catch {
...
function abrirCreador(fecha) {
  ...
  drawer.innerHTML = `
    ...
    <section>
      ...
      <button class="btn-ghost" id="creadorIdea" style="margin-top:8px">${icl("chispa")} Sugerir idea</button>
    </section>
    <section>
      <h4>Formato</h4>
...
async function abrirActividadMeta() {
  ...
  const fmt = t => {
    if (!t) return "";
    // Meta manda fechas ISO, horas programadas en segundos y nuestro ts en ms
    const f = new Date(typeof t === "number" ? (t > 1e12 ? t : t * 1000) : t);
...
const VISTAS_ORDEN = MODO_CLIENTE
  ? ["aprobacion", "pipeline"]
  : ["aprobacion", "calendario", "historias", "rodaje", "feed"];
...
const PLANTILLAS_PRO = {
  musica: { n: "Música", ic: "musica", color: "#7A5CB8", desc: "De la idea a la canción publicada", fases: [
  ...
  diseno: { n: "Diseño", ic: "paleta", color: "#B07A28", desc: "Proyectos de marca y dirección de arte", fases: [
  ...
  personal: { n: "Personal", ic: "hoja", color: "#41684A", desc: "Metas propias con método y sin presión", fases: [
      ...
      <div class="pro-tipos" id="proTipos">
        ${Object.entries(PLANTILLAS_PRO).map(([k, t], i) => `
          <button data-tipo="${k}" class="${i === 1 ? "sel" : ""}">
            <span class="pro-ico" style="color:${t.color}">${icl(t.ic)}</span><b>${t.n}</b><span>${t.desc}</span>
          </button>`).join("")}
      </div>
    </section>
    <section>
...
function historiasDeDia(marca, fechaIso, dow) {
  ...
  const plan = ((h.plan[marca] && h.plan[marca][dow]) || [])
    .filter(txt => !h.quitadas[histKey(fechaIso, marca, txt)]);
  const extras = h.extras[fechaIso + "|" + marca] || [];
  return plan.map(txt => ({ txt, extra: false })).concat(extras.map(txt => ({ txt, extra: true })));
}
function partesClave(k) {
  const resto = k.slice(11);
  const sep = resto.indexOf("|");
  return { iso: k.slice(0, 10), mk: resto.slice(0, sep), txt: resto.slice(sep + 1) };
}
// Mover una historia a otra fecha (el carrusel se reacomoda solo)
function moverHistoria(k, nuevaFecha) {
  const h = historiasStore();
  const { iso, mk, txt } = partesClave(k);
  if (!nuevaFecha || iso === nuevaFecha) return k;
  const k2 = histKey(nuevaFecha, mk, txt);
  const key = iso + "|" + mk;
  if ((h.extras[key] || []).includes(txt)) {
    h.extras[key] = h.extras[key].filter(t => t !== txt);
    if (!h.extras[key].length) delete h.extras[key];
  } else {
    h.quitadas[k] = true; // era del plan semanal: se oculta solo ese día
  }
  const d2 = new Date(nuevaFecha + "T12:00:00");
  const dow2 = (d2.getDay() + 6) % 7;
  const enPlanDestino = ((h.plan[mk] && h.plan[mk][dow2]) || []).includes(txt);
  delete h.quitadas[k2];
  if (!enPlanDestino && !(h.extras[nuevaFecha + "|" + mk] || []).includes(txt)) {
    (h.extras[nuevaFecha + "|" + mk] = h.extras[nuevaFecha + "|" + mk] || []).push(txt);
  }
  if (h.prog[k]) { h.prog[k2] = h.prog[k]; delete h.prog[k]; }
  if (h.hechas[k]) { h.hechas[k2] = true; delete h.hechas[k]; }
  if (typeof videoURLs !== "undefined" && videoURLs[k]) { videoURLs[k2] = videoURLs[k]; delete videoURLs[k]; }
...
function renderHistorias() {
                    ...
                    <div class="hs-ring ${hecha ? "vista" : ""}">
                      <div class="hist-story ${hecha ? "hecha" : ""}" data-hist="${esc(k)}" role="button" tabindex="0" title="${esc(t.resto)}">
                        ${img
                          ...
                          : video && pr.poster
                          ? `<img class="hs-img" src="${pr.poster}" alt="">
                             <span class="hs-grad"></span>
                             <span class="hist-play hs-play-centro">${iconoHist('<path d="M8.5 5.5v13l10-6.5-10-6.5z"/>', "#fff", "hist-ic hist-ic-play")}</span>
                             <span class="hs-txt">${esc(t.resto)}</span>`
                          : video
                          ? `<div class="hs-placeholder hs-es-video" style="--tc:${t.color};--tb:${t.color}14">
                               <span class="hist-play">${iconoHist('<path d="M8.5 5.5v13l10-6.5-10-6.5z"/>', "#fff", "hist-ic hist-ic-play")}</span>
                               <span class="hs-ph-txt">${esc(t.resto)}</span>
                               <span class="hs-ph-formato hs-ph-video">${esc(video)}</span>
                             </div>`
                        ...
                        ${pr ? `<span class="hs-hora ${img || video ? "" : "falta"}" title="${img || video ? "A esta hora la subes tú — te llega un aviso al celular" : "Aún falta la imagen o el video"}">${pr.hora || "12:00"}</span>` : ""}
  ...
  // Los gestos SOLO mueven el carrusel — la semana se cambia únicamente con
  // las flechas. En el borde, el gesto se absorbe (nada de "atrás" del
  // navegador ni saltos de semana accidentales): overscroll-behavior en CSS
  // más este freno para los navegadores que aún propagan la rueda.
  el.querySelectorAll(".hist-rail").forEach(rail => {
    rail.addEventListener("wheel", e => {
      if (Math.abs(e.deltaX) <= Math.abs(e.deltaY)) return;
      const alInicio = rail.scrollLeft <= 1 && e.deltaX < 0;
      const alFinal = rail.scrollLeft >= rail.scrollWidth - rail.clientWidth - 1 && e.deltaX > 0;
  ...
  el.querySelectorAll("[data-quitar-hist]").forEach(b => {
    b.onclick = e => {
...
function abrirHistoriaClave(k, medio) {
  const iso = k.slice(0, 10);
  const resto = k.slice(11);
  const sep = resto.indexOf("|");
  openHistoria(iso, resto.slice(0, sep), resto.slice(sep + 1), medio);
}
// La entrada de programación se crea sola al primer gesto (imagen, video, hora…)
function progDe(k) {
  const h = historiasStore();
  if (!h.prog[k]) h.prog[k] = { auto: true, red: "ambas", hora: "12:00", img: null };
  h.prog[k].auto = true;
  return h.prog[k];
}
// Selector de video: solo toma el NOMBRE del archivo — el video queda en la
// carpeta "Videos Contenido Hub" del Mac y el trabajador lo sube desde ahí.
const videoPicker = document.createElement("input");
videoPicker.type = "file";
videoPicker.accept = "video/mp4,video/quicktime,.mp4,.mov";
videoPicker.hidden = true;
document.body.appendChild(videoPicker);
let videoPickTarget = null;
// URLs locales de los videos elegidos EN ESTA SESIÓN: permiten la
// previsualización en tiempo real (como el compositor de Meta).
const videoURLs = {};
// …y además cada video elegido se guarda en el navegador (IndexedDB) para
// que la previsualización sobreviva recargas: se elige UNA vez y listo.
function abrirBDVideos() {
  return new Promise((res, rej) => {
    const req = indexedDB.open("hubVideos", 1);
    req.onupgradeneeded = () => req.result.createObjectStore("videos");
    req.onsuccess = () => res(req.result);
    req.onerror = () => rej(req.error);
  });
}
async function guardarVideoLocal(nombre, blob) {
  try {
    const db = await abrirBDVideos();
    const tx = db.transaction("videos", "readwrite");
    const st = tx.objectStore("videos");
    st.put({ blob, ts: Date.now() }, String(nombre).toLowerCase());
    // limpieza: los videos de hace más de 21 días ya salieron — fuera
    st.openCursor().onsuccess = e => {
      const c = e.target.result;
      if (!c) return;
      if ((c.value.ts || 0) < Date.now() - 21 * 86400000) c.delete();
      c.continue();
    };
  } catch {}
}
function cargarVideoLocal(nombre) {
  return new Promise(res => {
    if (!nombre) return res(null);
    abrirBDVideos().then(db => {
      const req = db.transaction("videos").objectStore("videos").get(String(nombre).toLowerCase());
      req.onsuccess = () => res(req.result && req.result.blob ? URL.createObjectURL(req.result.blob) : null);
      req.onerror = () => res(null);
    }).catch(() => res(null));
  });
}
// Captura un frame del video como portada (viaja con Guardar cambios)
function posterDeVideo(file) {
  return new Promise(res => {
    ...
    const listo = () => {
      try {
        const f = Math.min(1, 960 / (v.videoHeight || 960));
        const c = document.createElement("canvas");
        c.width = Math.round((v.videoWidth || 540) * f);
        c.height = Math.round((v.videoHeight || 960) * f);
        c.getContext("2d").drawImage(v, 0, 0, c.width, c.height);
        res({ poster: c.toDataURL("image/jpeg", 0.72), url });
      } catch { res({ poster: null, url }); }
    };
    v.onloadeddata = () => { try { v.currentTime = Math.min(0.6, (v.duration || 1) / 3); } catch { listo(); } };
    v.onseeked = listo;
    v.onerror = () => res({ poster: null, url });
    setTimeout(() => { if (!v.videoWidth) res({ poster: null, url }); }, 6000);
  });
}
videoPicker.onchange = async () => {
  const f = videoPicker.files[0];
  if (!f || !videoPickTarget) return;
  const k = videoPickTarget;
  const pr = progDe(k);
  pr.video = f.name;
  pr.img = null;
  const { poster, url } = await posterDeVideo(f);
  pr.poster = poster;
  videoURLs[k] = url;
  guardarVideoLocal(f.name, f); // queda en el navegador: no hay que re-elegirlo
  marcarPendiente(); save(); renderHistorias();
  abrirHistoriaClave(k, "video");
};
function openHistoria(iso, mk, txt, medioVista) {
  const h = historiasStore();
  ...
  drawer.innerHTML = `
    ...
    <div class="pro-drawer-top">
      ...
      <div style="flex:1;min-width:0">
        ...
        <div class="sub" style="margin:2px 0 0">${m.nombre} · ${dia.toLowerCase()} ${num}${hecha ? " · publicada" : lista ? ` · planificada para las ${(pr && pr.hora) || "12:00"} — la subes tú` : ""}</div>
      </div>
    </div>

    <section class="pub-paso pub-auto" style="margin-top:18px">
      <h4><span class="paso-num auto">A</span> Planificar la historia</h4>
      <div class="hist-medio-fila">
        ...
        ${pr ? `<button class="hist-quitar-prog" id="histQuitarProg">Quitar planificación</button>` : ""}
      </div>

      ${medio === "imagen" ? `
      <p class="pub-nota">El diseño final en 1080×1920. A la hora planificada te llega un <b>aviso al celular</b> con la imagen lista para subirla.</p>
      <div class="hist-prog-grid">
        ...
        <div class="hist-prog-campos">
          ...
          <p class="hist-abrir-hint">${pr && pr.img
            ? "En el celular: elige Instagram → Historias y la imagen llega lista al editor — música y stickers a tu gusto."
            : "Desde el celular abre la cámara de historias. Sube la imagen y te la envía lista."}</p>
        </div>
      </div>`
      : `
      <p class="pub-nota">El video queda guardado en este navegador para <b>previsualizarlo con sonido</b> antes de subirlo desde Meta Business o la app.</p>
      <div class="hist-prog-grid">
        ...
        <div class="hist-prog-campos">
          <button class="btn-primary" id="histElegirVideo">${pr && pr.video ? "Cambiar video" : "Elegir el video…"}</button>
          <input id="histVideoNombre" class="edit-input" placeholder="nombre-del-archivo.mp4" value="${esc((pr && pr.video) || "")}" autocomplete="off" spellcheck="false">
          ${campoHoraRedes(pr, iso)}
        </div>
      </div>`}
      ...
      <p class="${lista ? "hist-estado-ok" : "pub-aviso"}" style="margin-top:12px">
        ${lista
          ? `Planificada: el ${dia.toLowerCase()} ${num} a las ${(pr && pr.hora) || "12:00"} te llega el aviso al celular para subirla.`
          : `Sube la imagen o el video y elige día y hora — quedará en el calendario y te avisará.`}
      </p>
  ...
  if (bPlay) bPlay.onclick = async () => {
    const marco = drawer.querySelector("#histVideoMarco");
    let url = videoURLs[k];
    if (!url && pr && pr.video) {
      url = await cargarVideoLocal(pr.video); // guardado en el navegador
      if (url) videoURLs[k] = url;
    }
    if (url) {
      marco.classList.add("reproduciendo");
      marco.innerHTML = `<video class="hist-video-el" src="${url}" controls loop playsinline></video>`;
      const v = marco.querySelector("video");
      v.volume = 1;
      v.play().catch(() => {});
    } else {
      toastVivo("Elige el archivo una vez y quedará guardado para reproducirlo siempre");
      videoPickTarget = k;
      videoPicker.value = "";
      videoPicker.click();
    }
  ...
  if (inVid) inVid.onchange = () => {
    const pr2 = progDe(k);
    const nuevo = inVid.value.trim() || null;
    if (nuevo !== pr2.video) pr2.poster = null; // otro archivo: el frame anterior ya no aplica
    pr2.video = nuevo;
    if (pr2.video) pr2.img = null;
    marcarPendiente(); save(); renderHistorias();
    openHistoria(iso, mk, txt, "video");
  };
  const inHora = drawer.querySelector("#histHora");
  if (inHora) inHora.onchange = () => {
    progDe(k).hora = inHora.value || "12:00";
    marcarPendiente(); save(); renderHistorias();
  ...
  const bQP = drawer.querySelector("#histQuitarProg");
  if (bQP) bQP.onclick = () => { delete h.prog[k]; marcarPendiente(); save(); renderHistorias(); openHistoria(iso, mk, txt); };
...
function abrirExtraHistoria(iso, mk) {
  ...
  drawer.innerHTML = `
    ...
    <section>
      <h4>Día</h4>
      <input type="date" id="extraFecha" class="edit-input" value="${iso}">
    </section>
    <section>
  ...
  const agregar = () => {
    ...
    const fechaElegida = drawer.querySelector("#extraFecha").value || iso;
    const key = fechaElegida + "|" + mk;
    (h.extras[key] = h.extras[key] || []).push(txt);
    marcarPendiente(); save();
    closeDrawer();
    renderHistorias();
    toastVivo(`Historia agregada al ${Number(fechaElegida.slice(8))} · ${MARCAS[mk].nombre}`);
  };
...
if (MODO_CLIENTE) {
  ...
  fab.innerHTML = `<span class="guardar-ico" aria-hidden="true">${icl("nube")}</span><span class="guardar-txt">Todo guardado</span><span class="guardar-punto" aria-hidden="true"></span>`;
  ...
  const kAviso = new URLSearchParams(location.search).get("historia");
  if (kAviso && kAviso.length > 11) {
    activarVista("historias");
    setTimeout(() => abrirHistoriaClave(kAviso), 350);
  }
}
